...
namespace Steam {
    ...
    QString getGamePath(const QString& game, const QString& def) {

        // Search all install folders for a given game, return def if not found

        for (const QString& folder : getSteamInstallFolders()) {
            QStringList entries = QDir(folder + "/SteamApps/common").entryList(QStringList(game), QDir::Dirs);

            if (!entries.empty() && QDir(folder + "/SteamApps/common/" + entries.at(0)).count()) {
